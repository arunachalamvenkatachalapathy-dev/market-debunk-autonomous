"""
Video processor ΓÇö stitches clips, burns subtitles, overlays mascots, normalizes audio.
...
def generate_ass_file(processed_scenes, total_duration, subtitle_style=None, ass_path=None):
    ...
    if not subtitle_style:
        ...
        margin_v = 960
        alignment = 5
        logger.info(f"≡ƒô¥ Using Hormozi-style subtitle overrides: {font_name} {font_size}pt, MarginV={margin_v}")
    ...
    logger.info(f"Subtitles written to {ass_path} ΓÇö {len(dialogue_lines)} events, {len(all_emphasis_words)} emphasis words")
    return ass_path
...
def assemble_final_video(processed_scenes, subtitle_style=None, assembly_config=None, mascot_timeline=None):
    ...
    """
    logger.info("=== ≡ƒÄ¼ Assembling Final Video ===")
    ...
    if assembly_config:
        ...
        logger.info(f"≡ƒÄ¼ Using PE assembly config: loudness={loudness_i}LUFS, logo={logo_scale}px, fps={assembly_config.get('output_fps', 25)}")
    ...
    logger.info(f"Γ£à Final output video created at {final_output}")
